...
import { rateLimit, rateLimitResponse, readJsonBody } from "@/lib/security";
...
// Conversational interview. Not currently called by any page — the live
// interview uses the fixed five questions in app/interview/page.tsx — but it was
// reachable by anyone: no auth, no rate limit, no size cap, on a handler that
// spends Anthropic credits per request. That is an open bill and a free
// general-purpose model proxy (the whole conversation comes from the caller).
export async function POST(request: Request) {
  ...
  try {
    ...
    // Signed-in callers only, and rate limited per account rather than per IP —
    // a hostel or campus NAT shares one address.
    const token = (request.headers.get("authorization") || "").replace(/^Bearer\s+/i, "");
    if (!token) return Response.json({ error: "Unauthorized." }, { status: 401 });
    const { data: { user }, error: authErr } = await supabaseAdmin.auth.getUser(token);
    if (authErr || !user) return Response.json({ error: "Unauthorized." }, { status: 401 });

    const rl = rateLimit("interview", user.id, 60, 60 * 60_000);
    if (!rl.ok) return rateLimitResponse(rl.retryAfter);

    const typeContext = TYPE_CONTEXT[body.relationshipType ?? ""] || TYPE_CONTEXT["Still Figuring Out"];

    // The old code called history.map() straight off the body: a missing or
    // non-array history threw a TypeError and surfaced as a 500.
    ...
    const history = body.history
      ...
      .slice(-40)
      .map(m => ({ role: m.role === "ai" ? "ai" : "user", text: m.text.slice(0, 4000) }));

    // The Messages API requires the first turn to be a user turn, so drop any
    // leading assistant entries rather than sending a request it will reject.
