...
export async function POST(request: Request) {
  const bodyResult = await readJsonBody<Record<string, unknown>>(request, 16_384);
  ...
  try {
    const personA        = sanitiseRecord(body.personA as Record<string, unknown> || {});
    const personB        = sanitiseRecord(body.personB as Record<string, unknown> || {});
    const relationshipType = sanitise(body.relationshipType);

    // This route has no login and no age gate — the content screen is the only
    // thing standing between free text and a report that echoes it back.
    const modCheck = await checkContentSafety([...Object.values(personA), ...Object.values(personB)]);
    if (modCheck.flagged) {
      return Response.json({ error: "One of the profiles couldn't be processed — please keep entries respectful and free of explicit or violent content." }, { status: 400 });
    }
