...
import { matchScore, buildMatchReason, type MatchProfile } from "@/lib/matcher";
import { getResearchBaseline } from "@/lib/researchBaseline";
...
export async function POST(req: Request) {
  ...
  try {
    const profile = sanitiseRecord(body.profile as Record<string, unknown> || {});
    ...
    // This route has no login and no age gate — the content screen is the
    // only thing standing between free text and a score reflected back to the
    // caller, scored against real registrants' data.
    const modCheck = await checkContentSafety(Object.values(profile));
    if (modCheck.flagged) {
      return Response.json({ error: "flagged", message: "Please keep your entries respectful and free of explicit or violent content." }, { status: 400 });
    }
